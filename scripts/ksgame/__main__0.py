...
class ModalTimerOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        print(f'in ModalTimerOperator/modal')
#        print(f'self.fsw.key_input: {self.fsw.key_input}')
#        if self.fsw.key_input in {'A', 'D'}:
#        showTxt(f"in ModalTimerOperator.modal, global key_input:{self.key_input}")

#        print(f"in ModalTimerOperator.modal, global key_input:{key_input}")
        global key_input
        print(f'in ModalTimerOperator/modal global key_input: {key_input}')

        # 6/14 blender system console
        #self.key_handling being called
        #in key_handling
        #in ModalTimerOperator/modal
        #in ModalTimerOperator/modal global key_input: D
        #in ModalTimerOperator/modal/ if key_input == D
        
        if key_input in {'A', 'D'}:
#            showTxt(f"ModalTimerOperator.modal, global key_input:{key_input}")
#            key_input = self.fsw.key_input
            print(f'in ModalTimerOperator/modal/if key_input in A, D: key_input = {key_input}')
#            print(f'self.fsw.key_input: {self.fsw.key_input}')
            print(f'in ModalTimerOperator/modal/if key_input in A, D: self.key_handling being called')
            self.key_handling(context, event, key_input)
            print(f'in ModalTimerOperator/modal/if key_input in A, D: self.key_handling called')
